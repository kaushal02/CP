...
inline matrix pwr(matrix a, int n) {
    assert(n >= 0);
    matrix ans;
    ans.id();
    while(n) {
...
template<typename T>
inline void pwr(T a[Nmat][Nmat], int n) {
    assert(n >= 0);
    T temp[Nmat][Nmat];
...
*******************************************************************************/
const int Nmat = 5;
int pow2[65][Nmat][Nmat], base[Nmat], temp_vec[Nmat];
inline void seed() {
...
inline void store() {
    rep1(x,60)rep(i,Nmat)rep(j,Nmat) {
        rep(k,Nmat) pow2[x][i][j] += pow2[x-1][i][k] * pow2[x-1][k][j] % mod;
        pow2[x][i][j] %= mod;
    }
}
inline void mul(int vec[Nmat], int depth) {
    mem(temp_vec, 0);
    rep(i,Nmat) {
