...
inline void add(ll m, ll b) {
    ...
    while(sz > 1 and (double)(b - B[sz-2]) / (M[sz-2] - m) < (double)(B[sz-1] - B[sz-2]) / (M[sz-2] - M[sz-1])) {
        M.pop_back();
        B.pop_back();
        sz--;
    }
...
inline ll query(ll x) {
    while(pt < sz-1 and x > (double)(B[pt] - B[pt+1]) / (M[pt+1] - M[pt]))
        pt++;
    return M[pt] * x + B[pt];
}
...
inline ll query(ll x) {
    ...
    while(l < r) {
        ...
        if(x * M[m1] + B[m1] operator x * M[m2] + B[m2])
            l = m1;
        else
            r = m2;
    }
